...
const TokenPreview = ({ type, value }: { type: string; value: string | number }) => {
...
const GroupNode = ({ data }: NodeProps<TokenNodeData>) => {
                ...
                {isExpanded && (
                    <div className="token-expansion-area" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="token-list-v2">
                            {visibleTokens.map((token: TokenNodeData, i: number) => (
                                <div key={i} className="token-list-item-v2">
                                    <div className="item-main">
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                            <TokenPreview type={token.type ?? 'other'} value={token.value} />
                                            <span className="item-name">{token.label}</span>
                                        </div>
                                        <span className="item-type">{token.type}</span>
                                    </div>
                                    <div className="item-path">{String(token.value)}</div>
                                </div>
                            ))}
                        </div>
...
interface MapCanvasProps {
    ...
    edges: Edge[];
    onNodeClick?: (event: React.MouseEvent, node: Node<TokenNodeData>) => void;
